//
// Created by Subangkar on 08-Dec-19.
//
...
inline segtrnode_t seg_lazy_val_reset(segtrnode_t &lazy_node) {
	return lazy_node = SEG_TREE_NULL_PENDING;
}
...
inline segtrnode_t seg_tree_val_update(segtrnode_t &tree_node, segtrnode_t pending_val, int l, int r) {
	return tree_node += (r - l + 1) * pending_val;
}
...
segtrnode_t updateRangeUtil(segmenttree_t tree, segmenttree_t lazy, int si, int ss, int se, int us,
	...
	if (lazy[si] != SEG_TREE_NULL_PENDING) {
		...
		// nodes
//		tree[si] += lazy[si];///////////////////////
		seg_tree_val_update(tree[si], lazy[si], ss, se);
...
//		tree[si] += diff;
		seg_tree_val_update(tree[si], diff, ss, se);
...
//				printf("%d\n", v);
				...
				cin >> l >> r;
				cout << getRangeVal(tree, lazy, N, l, r) << endl;
			}
		}
	}
	return 0;

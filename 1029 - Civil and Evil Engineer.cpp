...
public:
	...
	int MaxST_Prim(int source, Graph& Tree);
	int MaxST_Prim_Execute(Graph& Tree, MinHeap<int>& Q, int key[], int parent[]);
...
int Graph::MaxST_Prim(int source, Graph & Tree)
{
	...
	return MaxST_Prim_Execute(Tree, Q, key, parent);
}

int Graph::MaxST_Prim_Execute(Graph & Tree, MinHeap<int>& Q, int key[], int parent[])
{
	int maxCost = 0;
...
int main()
{
	...
	while (cs < t)
	{
		...
		while (scanf("%d %d %d", &u, &v, &w) != EOF && u != 0 || v != 0 || w != 0)
		{
			g.addEdge(u, v, w);
		}
		...
		Graph MaxSt(directed);
		int maxCost = g.MaxST_Prim(source, MaxSt);
		int totalCost = minCost + maxCost;
